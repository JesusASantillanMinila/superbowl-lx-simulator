...
if st.button(f"🚀 Run Super Bowl Simulation", use_container_width=True):
    ...
    with col1:
        ...
        # --- 2) LABELED PROGRESS GRAPH ---
        st.write(f"**{afc_choice}** ({afc_win_pct:.1f}%) vs **{nfc_choice}** ({(100 - afc_win_pct):.1f}%)")
        st.progress(afc_win_pct / 100)
        st.caption(f"Left: {afc_choice} | Right: {nfc_choice}")
    ...
    # --- 3) LABELED POINT SPREAD DISTRIBUTION ---
